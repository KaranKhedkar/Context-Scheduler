...
import { Layers, CheckCircle2, CopySlash } from 'lucide-react';

export default function ComparisonView({ queryResult }) {
  if (!queryResult) {
    return (
      <div className="p-12 text-center text-slate-500 bg-slate-900/40 rounded-xl border border-slate-800/60 shadow-lg shadow-black/20">
        <Layers className="w-8 h-8 mb-3 mx-auto text-slate-600 animate-pulse" />
        <h3 className="text-sm font-semibold text-slate-200 mb-1">No Active Query Comparison</h3>
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="bg-slate-900/60 border border-slate-800/60 rounded-xl p-5 shadow-lg shadow-black/25 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <h2 className="text-sm sm:text-base font-bold text-white mt-1">"{query}"</h2>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-slate-900/60 border border-emerald-500/35 rounded-xl p-5 shadow-lg shadow-emerald-950/20 space-y-4">
          ...
          <div className="space-y-2.5">
            {schedulerMemories.map((mem, idx) => (
              <div key={mem.id || idx} className="p-3.5 rounded-xl bg-slate-950/70 border border-slate-800/80 hover:border-slate-700 transition-all duration-200 space-y-1.5 shadow-sm">
          ...
          {deduplicatedMemories.length > 0 && (
            <div className="p-3 rounded-xl bg-slate-950/60 border border-amber-500/30 text-amber-300 text-xs flex items-center gap-2.5 shadow-inner">
        ...
        {/* Column 2: Vector RAG */}
        <div className="bg-slate-900/40 border border-slate-800/60 rounded-xl p-5 shadow-lg shadow-black/20 space-y-4">
          <div className="flex items-center justify-between border-b border-slate-800/80 pb-3">
            <div className="flex items-center gap-2.5">
          ...
          <div className="space-y-2.5">
            {ragMemories.map((mem, idx) => (
              <div key={mem.id || idx} className="p-3.5 rounded-xl bg-slate-950/40 border border-slate-800/60 space-y-1.5 shadow-sm">
                <div className="flex items-center justify-between text-xs">
      ...
      {/* Core Key Finding Banner */}
      <div className="bg-slate-900/60 border border-slate-800/60 rounded-xl p-4 text-center text-xs sm:text-sm text-slate-200 shadow-md shadow-black/20">
        {deduplicatedMemories.length > 0 ? (
          <span className="leading-relaxed">
